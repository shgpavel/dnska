...
test_parse_root_name(void)
{
	...
	assert(dns_parse_name(buf, sizeof(buf), 0, out, sizeof(out), &consumed) == 0);
	assert(consumed == 1);
	assert(strcmp(out, "") == 0);
}
...
test_parse_name_pointer_failures(void)
{
	...
	assert(dns_parse_name(loop_buf, sizeof(loop_buf), 0, out, sizeof(out), NULL) < 0);
	assert(dns_parse_name(eob_buf, sizeof(eob_buf), 0, out, sizeof(out), NULL) < 0);
	assert(dns_parse_name(hop_buf, sizeof(hop_buf), 0, out, sizeof(out), NULL) < 0);
}
...
test_opt_cookie_ignored_in_cache_key(void)
{
	...
	pos_a += 4;
	pos_a = append_opt_record(buf_a, pos_a, 1232, 0, cookie_a, sizeof(cookie_a));
	...
	pos_b += 4;
	pos_b = append_opt_record(buf_b, pos_b, 1232, 0, cookie_b, sizeof(cookie_b));
...
test_response_match_uses_question_section_only(void)
{
	...
	write_u16(response_buf + 2, DNS_FLAG_QR | DNS_FLAG_RD | DNS_FLAG_RA);
